...
#define WORD_ADDED_LENGTH 20 // taille a rajouter en cas de depassement
...
//cword * new_cword(const char * word, int source); why not une fois que tt marche
...
int main(int argc, char *argv[]) {
  ...
  while (f <= total_files || (no_file && f <= total_files)) {
    ...
    if (no_file || (argc == total_files && !no_file)) {
      HIGHLIGHT("---reading from file number", f);
      file = stdin;
    ...
    while ((ch = fgetc(file)) != EOF) {
      if (isspace(ch)) {     //c'est ca a 100% || ispunct(ch) pour -p
        if (index_word > 0) {
          word[index_word] = '\0';
          //si jamais ya eu un realloc pour le mot remettre en nrml apres le mot agi
          taille_mot = WORD_LENGTH_INITIALE;
          cword ctest;
          ctest.mot = word;
          void *recherche = bst_search(xwbst, ctest); //on cherche le mot de
          // ctest
          ...
          if (recherche == NULL) {
            ...
            strcmp(final_cword,word);

            void *t = bst_add_endofpath(xwbst, final_cword); //on ajoute cell ihi
            if (t == NULL) {
              fprintf(stderr, "erreur lors de l'ajoute du mot %s", word);
              goto error_capacity;
            }
            if (holdall_put(xwha, final_cword) != 0) { //on ajoute au fourtout
              goto error_capacity;
            }
          } else { //ya deja le mot on doit reutiliser cell pour apres
            if (SOURCE(recherche) != f) {
              SOURCE(recherche) = -1;
            } else { //donc la meme source
              OCC(recherche) += 1; //ou t kifkif je crois
            }
            //free(cell->mot);
            //free(cell); // pas ca car je veux reutiliser
          }
        }
        /////////////////////
      } else {
    ...
    if (file != stdin) {
      ...
      clearerr(stdin);
      HIGHLIGHT("---end reading from file number", f);
      fprintf(stdout, "\n");
      if (argc == f) {
        break;
      }
...
int reverse_compar_occ(const void *c1, const void *c2) {
  return -(compar_occ(c1, c2));
}

...
#define ANSI_BG_RED "\033[41m"  // Définit la couleur de fond en rouge
...
int main(int argc, char *argv[]) {
  ...
  bool options_ended = false; // pour lire le reste comme non option des ya une non option

//-----------------------GESTION DES ARGUMENTS------------------------------------
  for (int i = 1; i < argc; i++) {
    char *arg = argv[i]; //argument en cours de traitement de la ligne de commande
    ...
    if (strcmp(arg, SHORT_HELP) == 0 || strcmp(arg, LONG_HELP) == 0) {
      help(); // affichage de l'aide si --help ou -?
      return r;
  ...
  int total_files = argc - opt_count - 1;
    ...
    no_file = true;
    total_files = 1;  //pour dire que ya que des options ou c'est vide
  }
  //fprintf(stdout, " f = %d\n", f);
  //fprintf(stdout, " total files = %d\n", total_files);
  //fprintf(stdout, " argc = %d\n", argc);
  //fprintf(stdout, " optcount = %d\n", opt_count);
  //fprintf(stdout, " stdin count = %d\n", stdin_count);
